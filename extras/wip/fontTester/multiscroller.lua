...
onContentTouch = function( self, event )
	--print(event.phase, dragMode, isInBounds( event, self ))

--[[	
--print("dragMode == " .. dragMode)
--EDOCHI 	if(throwing) then return false end -- Stay out while throwing EFM
	if( self.selFill_hack and event.phase == "began" ) then
		self:setFillColor( unpack(self.selFill_hack) )
	end
	if( self.unselFill_hack and (event.phase == "ended" or isInBounds( event, self ) == false )) then
		self:setFillColor( unpack(self.unselFill_hack) )
	end
--]]	
