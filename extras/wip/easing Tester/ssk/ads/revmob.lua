...
return e
end,payloadAsJsonString=function(e,t)return s.encode(n.payload(e,t))end,printEnvironmentInformation=function(r,i,l,u)e.debug("==============================================")e.debug(n.payloadAsJsonString(i,l))local d=nil
local c=nil
local s=nil
...
end)package.preload['revmob_client']=(function(...)local h=require('json')local g=require('socket.http')local a=require("ltn12")local e=require('revmob_about')local n=require('revmob_log')local t=require('revmob_messages')local o=require('revmob_events')local u=require('revmob_context')local m=require('revmob_cache')local i=require('revmob_device')local d=require('revmob_utils')REVMOB_ID_IOS='iPhone OS'REVMOB_ID_ANDROID='Android'local e
local v=30
local p='https://ios.revmob.com'local c='https://android.revmob.com'local L="app"local D=false
local k=true
...
local function _(e)return r().."/api/v4/mobile_apps/"..e.."/sessions.json"end
...
local function b(e)return e and string.len(e)==24
end
local function w(n)if n==nil then return nil end
local e
...
if e==nil then
e=n[REVMOB_ID_IOS]if not b(e)then
...
c(I(e.appId),n)end
local function r(r,a,i,l)local i=w(i)if e.sessionStarted then
if i~=nil then
n.info("Ad registered with Placement ID "..i)end
c(p(e.appId,r,a,i),l)else
...
local u
local function p(n)local t={}local e=""local i,e,t=g.request{method="POST",url=n,source=a.source.string(e),headers={["Content-Length"]=tostring(#e),["Content-Type"]="application/json"},sink=a.sink.table(t),}return u(n,e,t)end
...
e={TEST_WITH_ADS="with_ads",TEST_WITHOUT_ADS="without_ads",TEST_DISABLED=nil,appId=nil,appIds=nil,sessionStarted=false,testMode=nil,listenersRegistered=false,serverEndPoints={},ipAddress=nil,startSession=function(r,l)local t=w(r)if b(t)then
if not e.sessionStarted then
e.appId=t
e.appIds=r
local e=function(r)local a=r.status or r.statusCode
if(a==200)then
local a,r=pcall(h.decode,r.response)if(a~=nil and r~=nil)then
...
S()end
end,fetchFullscreen=function(n,e)r('fullscreen','fullscreens',n,e)end,fetchBanner=function(n,e)r('banner','banners',n,e)end,fetchLink=function(e,n)r('link','anchors',e,n)end,fetchPopup=function(e,n)r('pop_up','pop_ups',e,n)end,reportImpression=function(e)if e~=nil then
...
local i,e=pcall(h.decode,r.response)if(not i or e==nil)then
...
end,openWebView=function(n)l=display.newGroup()d.createImageFromBase64(d.CLOSE_BUTTON,d.CLOSE_BUTTON_IMAGE)local e=display.newImage(d.CLOSE_BUTTON_IMAGE,system.TemporaryDirectory)e.x,e.y=display.contentWidth-e.width/2,e.height/2+display.screenOriginY
...
e=""end
local i,e,t=g.request{method="POST",url=n,source=a.source.string(e),headers={["Content-Length"]=tostring(#e),["Content-Type"]="application/json"},sink=a.sink.table(t),}return u(n,e,t)end,getMarketURLWithoutFollow=function(e)f=false
local e=p(e)f=true
return e
...
end)package.preload['revmob_fullscreen_web']=(function(...)local l=require('revmob_log')local r=require('revmob_messages')local o=require('revmob_events')local n=require('revmob_client')require('revmob_sound_player')local t="fullscreen"local i
i={autoshow=true,listener=nil,impressionUrl=nil,clickUrl=nil,htmlUrl=nil,dspHtml=nil,campaignId=nil,new=function(e)local e=e or{}setmetatable(e,i)return e
end,load=function(e,r)e.networkListener=function(i)local i,r=n.theFetchSucceed(t,i,e.listener)if i then
local i=r['fullscreen']['links']e.impressionUrl=n.getLink('impressions',i)e.clickUrl=n.getLink('clicks',i)e.htmlUrl=n.getLink('html',i)l.release("Fullscreen loaded - "..e.campaignId)if e.listener~=nil then e.listener({type=o.AD_RECEIVED,ad=t})end
if e.autoshow then
e:show()end
end
end
...
l.info(r.OPEN_MARKET)if t then
...
if i.errorCode then
l.release("Error: "..tostring(i.errorMessage))end
return true
end
...
e.clickSoundURL=i
local t=t['fullscreen']['links']local o=n.getLink('impressions',t)local i=n.getLink('clicks',t)local s=n.getLink('html',t)local u=n.getLink('image',t)local f=n.getLink('close_button',t)local a=n.getLink('dsp_url',t)local t=n.getLink('dsp_html',t)local n=""local d=d.parse(i)local d=d["query"]if d~=nil then
...
n="Testing Mode"end
if t~=nil then
l.debug("DSP html fullscreen")e.createFile(t)e.view=r.new(e.params)e.view.dspHtml=t
...
e.view:show()end
elseif a~=nil then
l.debug("DSP url fullscreen")e.view=r.new(e.params)e.view.htmlUrl=a
e.view.impressionUrl=o
e.view.clickUrl=i
e.view.campaignId=n
e.view.autoshow=e.autoshow
...
e.view:show()end
elseif s~=nil then
l.debug("Rich fullscreen")e.view=r.new(e.params)e.view.htmlUrl=s
e.view.impressionUrl=o
e.view.clickUrl=i
e.view.campaignId=n
e.view.autoshow=e.autoshow
...
l.debug("Static fullscreen")e.view=c.new(e.params)e.view.imageUrl=u
...
end)package.preload['revmob_banner_web']=(function(...)local l=require('revmob_log')local a=require('revmob_messages')local r=require('revmob_events')local t=require('revmob_utils')local d=require('revmob_device')local n=require('revmob_client')require('revmob_sound_player')local o="banner"local i
...
local t=t['banners'][1]['links']e.impressionUrl=n.getLink('impressions',t)e.clickUrl=n.getLink('clicks',t)e.htmlUrl=n.getLink('html',t)l.release("Banner loaded - "..e.campaignId)if e.listener~=nil then e.listener({type=r.AD_RECEIVED,ad=o})end
...
t=n.getMarketURL(e.clickUrl)else
t=n.getMarketURLWithoutFollow(e.clickUrl)end
l.info(a.OPEN_MARKET)if t then
if e.openInside==true and e.appOrSite=="site"then
n.openWebView(t)else
system.openURL(t)end
end
...
end,update=function(e,n,i,t,l,o)e:setPosition(n,i)e:setDimension(t,l,o)end,release=function(e)if e.webView then
...
return i
end)package.preload['revmob_banner_static']=(function(...)local i=require('revmob_log')local s=require('revmob_messages')local r=require('revmob_events')local t=require('revmob_utils')local a=require('revmob_device')local n=require('revmob_client')require('revmob_sound_player')local l="banner"local o
...
e.image=o.target
local o=(t.Screen.width()>640)and 640 or t.Screen.width()local a=(a.isIPad()and 100 or 50*(t.Screen.bottom()-t.Screen.top())/display.contentHeight)local c=(t.Screen.left()+o/2)local d=(t.Screen.bottom()-a/2)e:setPosition(e.x or c,e.y or d)e:setDimension(e.width or o,e.height or a)e.image.tap=function(t)if not e._clicked then
e._clicked=true
...
n="Testing Mode"end
if t~=nil then
o.debug("DSP hmtl banner")e.createFile(t)e.view=r.new(e.params)e.view.dspHtml=t
...
o.debug("Static banner")e.view=u.new(e.params)e.view.imageUrl=c
e.view.impressionUrl=l
e.view.clickUrl=i
e.view.campaignId=n
e.view.autoshow=e.autoshow
...
function string:split(o,t)if not t then
t={}end
local e=1
local i,l=string.find(self,o,e)while i do
table.insert(t,string.sub(self,e,i-1))e=l+1
i,l=string.find(self,o,e)end
table.insert(t,string.sub(self,e))return t
end
n.fetchBanner(e.placementIds,r)end,hide=function(e)e.autoshow=false
if e.view~=nil then e.view:hide()end
end,show=function(e)e.autoshow=true
if e.view~=nil then e.view:show()end
...
function string:split(o,e)if not e then
e={}end
local n=1
local i,l=string.find(self,o,n)while i do
table.insert(e,string.sub(self,n,i-1))n=l+1
i,l=string.find(self,o,n)end
...
local l="/"local i=nil
local e=string.find(n,l,1)i=e
while e do
e=string.find(n,l,e+1)if e then
i=e
end
end
