...
_G.listen = function( name, listener ) 
  if( type(listener) == "function" ) then
    ...
    listener2 = function( event )
      if( listener.removeSelf == nil ) then
...
function _G.catchEndedTouches( obj )
  obj.touch = function(self,event) 
    if( event.phase == "ended" or event.phase == "cancelled" ) then return true end
    return false
  end
...
_G.fitText = function( obj, origText, maxWidth )
...
_G.getImageSize = function ( path, basePath )
...
_G.strip_Control_Codes = function( str )
   local s = ""
...
_G.strip_Control_and_Extended_Codes = function( str )
   local s = ""
   for i = 1, str:len() do
...
local function fixUTF8(s, replacement)
  ...
  while p <= len do
    if     p == string.find("[%z\1-\127]", p) then p = p + 1
    elseif p == string.find("[\194-\223][\128-\191]", p) then p = p + 2
    elseif p == string.find(       "\224[\160-\191][\128-\191]", p)
        or p == string.find("[\225-\236][\128-\191][\128-\191]", p)
        or p == string.find(       "\237[\128-\159][\128-\191]", p)
        or p == string.find("[\238-\239][\128-\191][\128-\191]", p) then p = p + 3
    elseif p == string.find(       "\240[\144-\191][\128-\191][\128-\191]", p)
        or p == string.find("[\241-\243][\128-\191][\128-\191][\128-\191]", p)
        or p == string.find(       "\244[\128-\143][\128-\191][\128-\191]", p) then p = p + 4
    else
